...
class VideoConverter(wx.Frame):
    ...
    def on_info_page(self, event):
        description = """\
Видеоконвертер — простая и быстрая программа для перекодирования видео с использованием аппаратного ускорения NVIDIA.
Программа является удобной графической обёрткой над мощным инструментом FFmpeg и оптимизирована специально для видеокарт NVIDIA.

Основные особенности:
Видео: кодируется с помощью аппаратного энкодера NVENC (H.264 / HEVC) — максимально быстрое кодирование с минимальной нагрузкой на процессор.
Аудио: всегда перекодируется в современный и широко поддерживаемый формат AAC (обычно 128–512k кбит/с, в зависимости от числа каналов).
На вход принимает файлы в следующих форматах: MKV, MP4, MOV, AVI.
Выходной контейнер: MP4
Возможность выбора качества.
Простой интерфейс: перетащите файлы → выбрали качество → нажали «Конвертировать».
Работает только на компьютерах с видеокартой NVIDIA (GeForce GTX/RTX, Quadro и др.), у которых поддерживается NVENC."""
